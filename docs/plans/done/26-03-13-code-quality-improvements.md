...
Remove the unused `components_params` parameter from `add_item!` signature. Also update `docs/references/models.md` — search for `components_params` (appears twice, in the Order methods section and code example) and replace the signature with `add_item!(product:, special_notes: nil)` in both places.
